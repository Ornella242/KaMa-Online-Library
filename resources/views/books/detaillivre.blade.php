    ...
    <section class="book-detail-page">
        <div class="container">
            <nav class="book-detail-breadcrumb">
                ...
                <span>/</span>
                @if($book->category)
                    <a href="{{ route('catalogue', ['category' => $book->category_id]) }}">{{ $book->category->name }}</a>
                    <span>/</span>
                @endif
                <em>{{ $book->title }}</em>
            </nav>
            ...
            <div class="book-detail-hero">
                <div class="book-detail-cover">
                    ...
                    <span class="book-detail-type">
                        <i class="bi {{ $isAudio ? 'bi-headphones' : 'bi-file-earmark-text' }}"></i>
                        {{ $isAudio ? 'Livre audio' : 'Ebook' }}
                    </span>
                </div>

                <div class="book-detail-info">
                    <span class="book-detail-category">{{ $book->category?->name ?? 'Sans catégorie' }}</span>
                    <h1>{{ $book->title }}</h1>
                    <p class="book-detail-author">
                        Par
                        <strong>{{ trim(($book->author?->firstname ?? '') . ' ' . ($book->author?->lastname ?? '')) ?: 'Auteur inconnu' }}</strong>
                    </p>
                    ...
                    <div class="book-detail-rating">
                        ...
                        @endfor
                        <span>{{ number_format($avgRating, 1) }} · {{ $reviewsCount }} avis</span>
                    </div>

                    <p class="book-detail-summary">{{ $book->short_description }}</p>

                    <div class="book-detail-meta">
                        @if($isAudio)
                            <span><i class="bi bi-clock"></i> {{ $book->duration ?: '—' }}</span>
                        @else
                            <span><i class="bi bi-file-text"></i> {{ $book->pages ?: 0 }} pages</span>
                        @endif
                        <span><i class="bi bi-calendar3"></i> {{ $book->publication_year }}</span>
                        <span><i class="bi bi-translate"></i> {{ $book->language }}</span>
                        @if($book->subcategory)
                            <span><i class="bi bi-tag"></i> {{ $book->subcategory->name }}</span>
                        @endif
                    </div>

                    <div class="book-detail-buy">
                        <div class="price">{{ number_format($book->price, 0, ',', ' ') }} <small>XOF</small></div>
